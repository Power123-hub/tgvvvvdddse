...
client.on("message", message => {
  if (message.content === `${PREFIX}help`) {
    var embed = new Discord.RichEmbed()
      ...
      .setColor(`${COLOR}`)
      .setDescription(
        `${PREFIX}play, ${PREFIX}skip, ${PREFIX}np, ${PREFIX}volume, ${PREFIX}stop, ${PREFIX}resume, ${PREFIX}queue, ${PREFIX}pause, ${PREFIX}clean`
      );
...
client.on("message", async msg => {
  ...
  if (command === "play") {
    ...
    msg.channel
      .send(`Cleared the queue`)
      .then(m => serverQueue.connection.dispatcher.clear());
    ...
    return msg.channel.send(
      `Song duration is - ${hours}h:${minutes}m:${seconds}s`
    );
  } else if (command === "np") {
    var embed = new Discord.RichEmbed()
